...
var viewDeptSales = function() {
	connection.query("SELECT department_id, department_name, over_head_costs, total_sales FROM departments", function(err, DBresults) {
		if (err) throw err;
		...
		DBresults.forEach(function(deptInfo) {
			...
			deptSalesTable.cell("Total Profit", deptInfo.total_sales, Table.number(2))
...
var createDept = function() {
	console.log("in progress, come back later");
	supervisorOptions();
